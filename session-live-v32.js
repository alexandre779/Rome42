...
function tick(){if(!live?.running||live.finished)return;const s=currentSeg(),e=elapsedSegment(),remaining=s?.durationSec?Math.max(0,s.durationSec-e):null;if(s?.durationSec){if(voiceMode()==='guided'&&!live.warned30&&remaining<=30&&remaining>28){live.warned30=true;speak('30 secondes restantes')}if(remaining<=0){next();return}}const clock=q('#r42LiveLayer .live-clock');if(clock)clock.textContent=remaining===null?fmtClock(e):fmtClock(remaining)}
